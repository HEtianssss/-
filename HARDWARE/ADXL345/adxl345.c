...
{					     
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_SetBits(GPIOB,GPIO_Pin_5|GPIO_Pin_4); 				//PB0,PB1 输出高
}
/************************************************************************
...
{				  
	...
	{  
		...
		ADXL345_Write_Single(OFSX,0x00);   		//X 偏移量
		ADXL345_Write_Single(OFSY,0x00);   		//Y 偏移量 
		ADXL345_Write_Single(OFSZ,0x00);   		//Z 偏移量
		return 0;
	}			
	return 1;	   								  
}   
...
void ADXL345_RD_XYZ(unsigned char *pBuffer)
{
    ...
    {
		*pBuffer = ADXL345_RecvByte();pBuffer++;       //BUF[0]存储0x32地址中的数据
...
static u8 ADXL345_flg = 0;  //0  //正常  1 //跌倒
/***********************
	判断当前状态
	角度判断
**************************/
void Angle(void)					//判断状态
{
  ...
  int	ADXL345_y=0;				//y轴的角度
	static uint32_t getcount = 0 ;
	uint32_t  tep = GetTick();
	
	if((tep -  getcount ) > 500)		//500ms一测，控制测量次数
	{
	   	getcount = tep;	
		  ADXL345_RD_XYZ(BUF);                         //接收数据缓存区 
		ADXL345_y=(BUF[1]<<8)+BUF[0];
 
		if(ADXL345_y < 100 || (ADXL345_y > 65536-100))
		{
			ADXL345_flg = 0;				//正常
		}
		else 
