...
void IIC_GPIO_Config(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	 //使能GPIOB时钟和复用功能（要先打开复用才能修改复用功能）
...
//显示中英文字符
void OLED_ShowCH2(u8 x, u8 y,u8 n)
{

	u32 j;	
					OLED_Set_Pos(x, y);
					for(j=0;j<16;j++)
						Write_IIC_Data(hz[n*32+j]);
					OLED_Set_Pos(x,y+1);
					for(j=0;j<16;j++)
						Write_IIC_Data(hz[n*32+j+16]);	
}
...
**u8 *bmp--图片数据
********************************************************/
void OLED_DrawBMP(u8 x,u8 y,u8 w,u8 h,const char *bmp)
{
	u8 i,j;
	u8 cnt=0;//记录该图片需要写多少页
	if(h%8)cnt=h/8;
	cnt=h/8;
	for(i=0;i<cnt;i++)//循环写的页数
	{
		OLED_Set_Pos(x,y+i);//设置光标位置
		for(j=0;j<w;j++)
		{
			Write_IIC_Data(bmp[j+w*i]);//写入一行数据
		}	
	}
}

void test_OLED4P(void)
{
//  OLED_Init();	
//	OLED_Clear();
...
////		RTC_Get();
//		sprintf(TS,"%02d:%02d:%02d",calendar.hour,calendar.min,calendar.sec);
//    OLED_ShowCH(32,0,(u8*)TS);
//		

//		

//	  sprintf(TS,"心率:%3d",Hrrest1.num);
//		OLED_ShowCH(0,2,(u8*)TS);
//		
//		sprintf(TS,"步数:%d",STEPS);
//		OLED_ShowCH(0,4,(u8*)TS);		

//		temperature=DS18B20_Get_Temp();	//读取温度		
//		sprintf(TS,"体温:%03.1f",(float)temperature/10) ;
//		OLED_ShowCH(0,6,(u8*)TS);
//		
//		
